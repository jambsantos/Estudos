### - Escreva uma função em python que lê dois números inteiros (a,b) e gere como resultado as seguintes 
# operações:adição, subtração, multiplicação e divisãoExecute a funçao na celula abaixo, deixando visivel exemplos de resultados.  
...
### - Crie uma função em python que calcule a média artimética de uma aluna que cursou a disciplina de Programação I. 
# A funcao deve receber como input as notas M1, M2 e M3 e calcular a média.
# Após a média calculada, a funcao deve anunciar se a aluna foi aprovado, reprovado ou pegou exame de acordo conm a seguinte classificacao:
...
equipes_dados = [BerthaLutz, GraceHopper, JaquelineGoes, SarahGilbert]
...
# RODAR A ETAPA 1
equipes_dados = ['BerthaLutz', 'GraceHopper', 'JaquelineGoes', 'SarahGilbert']
resultado_contagem = contar_a(equipes_dados)
...
def ordenacao(lista_alunas, equipes_dados):
    alunas_ordenadas = sorted(lista_alunas)
    equipes_alunas = {}

    for equipe in equipes_dados:
        equipes_alunas[equipe] = []

    equipe_atual = 0
    for aluna in alunas_ordenadas:
        if len(equipes_alunas[equipes_dados[equipe_atual]]) < 3:
            equipes_alunas[equipes_dados[equipe_atual]].append(aluna)
        else:
            equipe_atual += 1
            if equipe_atual >= len(equipes_dados):
                equipe_atual = 0
...
equipes_dados = ["BerthaLutz", "GraceHopper"]

equipes_alunas = ordenacao(lista_alunas, equipes_dados)
